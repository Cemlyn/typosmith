...
keyboard_layout = {
    ...
    'z': (0.6, 0), 'x': (1.6, 0), 'c': (2.6, 0), 'v': (3.6, 0), 'b': (4.6, 0), 'n': (5.6, 0), 'm': (6.6, 0)
}
...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if weight > 0 and (source, target) not in adjacent_pairs:  # Exclude adjacent pairs
...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if weight >= threshold and (source, target) not in adjacent_pairs:  # Only add edges above threshold and not adjacent
            G.add_edge(source, target, weight=weight)

# Calculate edge weights for visualization
edge_weights = [G[u][v]['weight'] for u, v in G.edges()]
max_weight = max(edge_weights)
edge_weights = [math.log(w/max_weight * 5 + 1) for w in edge_weights]  # Log scale for better visualization
...
#                             font_size=8,  # Smaller font for edge labels
#                             font_color='white',
#                             label_pos=0.5,  # Center of edge
#                             bbox=dict(facecolor='black', alpha=0.5, edgecolor='none'),  # Add background to labels
#                             ax=ax)

# Add title and adjust layout
ax.set_title(f"Typo Network (ignoring adjacent letters)", 
          fontsize=20, 
          pad=20,
          color='white')
...
cbar = plt.colorbar(sm, ax=ax, label='Connection Strength',
                   fraction=0.02,  # Make the colorbar shorter
                   pad=0.01)  # Reduce padding
cbar.ax.tick_params(labelsize=8)  # Make the tick labels smaller

# Save the plot
plt.savefig(os.path.join('data','letter_network_no_adjacent.png'), dpi=300, bbox_inches='tight', facecolor='black')
plt.close()