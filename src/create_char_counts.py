...
def is_diff_in_word(original: str, corrected: str) -> bool:
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        in_word = (before.isalpha() if before else False) and (after.isalpha() if after else False)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(os.path.join("data","github-typo-corpus.v1.0.0.csv"), encoding="utf-8")
    ...
    df['diff_in_word'] = df.apply(lambda row: is_diff_in_word(row['text'], row['corrected']), axis=1)
    ...
    word_pairs_df['non_alpha_percentage'] = word_pairs_df.apply(lambda row: sum(1 for c in row['correct'] if not c.isalpha()) / len(row['correct']), axis=1)
    word_pairs_df = word_pairs_df[word_pairs_df['non_alpha_percentage'] < 0.2]

    word_pairs_df['edit_distance'] = word_pairs_df.apply(lambda row: edit_distance(row['correct'], row['typo']), axis=1)

    # Remove rows where every letter is different, i.e. word substitution
    word_pairs_df['len_typo'] = word_pairs_df['typo'].apply(len)
    word_pairs_df = word_pairs_df[word_pairs_df['edit_distance'] <= word_pairs_df['len_typo']]

    word_pairs_df = word_pairs_df[~word_pairs_df['typo'].apply(lambda x: is_only_non_alpha(x))]
    word_pairs_df = word_pairs_df[~word_pairs_df['correct'].apply(lambda x: is_only_non_alpha(x))]
    ...
    word_pairs_df = word_pairs_df[~word_pairs_df['correct'].apply(lambda x: pd.isnull(x))]
    word_pairs_df = word_pairs_df[~word_pairs_df['typo'].apply(lambda x: pd.isnull(x))]

    word_pairs_df['has_consecutive_diffs'] = word_pairs_df.apply(lambda row: has_consecutive_diffs(row['correct'], row['typo']), axis=1)
    word_pairs_df['correct_len'] = word_pairs_df['correct'].apply(len)
    word_pairs_df['typo_len'] = word_pairs_df['typo'].apply(len)
    word_pairs_df['diff_count'] = word_pairs_df.apply(lambda row: diff_count(row['correct'], row['typo']), axis=1)

    single_char_typos = word_pairs_df[(~word_pairs_df['has_consecutive_diffs'])&(word_pairs_df['correct_len']==word_pairs_df['typo_len'])&(word_pairs_df['edit_distance']==1)&(word_pairs_df['correct_len']>1)]
