...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if (
            weight > 0 and (source, target) in adjacent_pairs
        ):  # Only include adjacent pairs
...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if (
            weight >= threshold and (source, target) in adjacent_pairs
        ):  # Only add edges for adjacent pairs
...
ax.set_title(f"Typo Network (adjacent keys only)", fontsize=20, pad=20, color="white")
...
# Save the plot
plt.savefig(
    os.path.join("data", "letter_network_adjacent.png"),
    dpi=400,
    bbox_inches="tight",
    facecolor="black",
)
