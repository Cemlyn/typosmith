...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if weight > 0 and (source, target) in adjacent_pairs:  # Only include adjacent pairs
            all_weights.append(weight)

# Set threshold to 75th percentile
threshold = np.percentile(all_weights, 0)
...
for source in char_counts:
    ...
    for target, weight in char_counts[source].items():
        if not target.isalpha():
            continue
        if weight >= threshold and (source, target) in adjacent_pairs:  # Only add edges for adjacent pairs
...
ax.set_title(f"Typo Network (adjacent keys only)", 
          fontsize=20, 
          pad=20,
          color='white')
...
# Save the plot
plt.savefig(os.path.join('data','letter_network_adjacent.png'), dpi=300, bbox_inches='tight', facecolor='black')
plt.close() 